...
#housing.lm.3 <- lm(log(housing_data.train.1$SalePrice)
               #    ~ .,data = housing_data.train.1)
#housing.lm.3
#plot(housing.lm.3)
```
...
Random Forest Model
Load in the dataset for preprocessing before training the random forest model. 
```{r}
# split the train data set in half
housing_data.train.1 <- housing_data.train[1:730,]
...
rf_model <- randomForest(
  SalePrice ~ .,          # Use all other columns as predictors
  data = housing_data.train.1,
  ntree = 500,            # Number of trees
  mtry = floor(sqrt(ncol(housing_data.train.1) - 1)),  # Number of features per split
