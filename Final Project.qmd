...
Now we check to ensure the Sale Price has normal distribution. We do this by viewing Q-Q plot. If the data follows a linear pattern with the trend line, then it has a normal distribution. If not, then we need to transform the data to ensure normal distribution.
...
Since the data does not follow normal distribution, we can apply the log function transform the data. This follows the paper we are references for this project.
...
As shown in lecture, we can view which variables are significant using a t test `t.test()`. Below, we use the Neighborhood field as an example. We can see which neighborhood will be more significant in our model. We also apply the t test on the Lot Area field, also removing the outliers of any houses over 50,000 sq ft.
...
Another way to measure significance is to use correlation `cor()`. We apply the correlation function on various fields in our data. The higher the correlation, the more significant the variable is.
...
First, we apply the `lm()` function to the living room area, which was shown to be a significant factor in the house sale price.
...
housing_data.train.noout <- housing_data.train.1[housing_data.train.1$LotArea < 10000,]
