...
print("Correlation between sale price and basement size:")
cor(log(housing_data.train.1$SalePrice),
    housing_data.train.1$X1stFlrSF)
```
Living room area and basement size seem to be one the highest correlated factors with sale price.
...
housing.lm.2 <- lm(log(housing_data.train.1$SalePrice) ~  housing_data.train.1$LotArea ,data=housing_data.train.noout)

housing.lm.2
plot(housing.lm.2)
```

Try adding in other features
```{r}
...
housing_data.train.noout <- housing_data.train.noout[housing_data.train.noout$GrLivArea < 4000,]

housing.lm.3 <- lm(log(housing_data.train.noout$SalePrice)
                   ~ + housing_data.train.noout$GrLivArea
...
plot(housing.lm.3)
```
Now we'll create a prediction with the other half of the training data set

```{r}
#view details of lm and predict sale price using the lm
summary(housing.lm.3)
# match dimensions
housing_predict <- predict.lm(housing.lm.3, housing_data.train.2[1:704,])
```
...
cor(housing_data.train.2$SalePrice[1:704],housing_predict)

```

```{r}
# visualize correlation
plot(housing_data.train.2$SalePrice[1:704]/1000, exp(housing_predict)/1000,
